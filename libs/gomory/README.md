# Библиотека gomory для TI-Nspire

Библиотека для решения задач целочисленного линейного программирования методом Гомори с пошаговым выводом.
...
### `gomory\gomory(mat, opt, intvars)`

Решает задачу целочисленного линейного программирования методом отсечений Гомори с пошаговым выводом.
...
- `opt` - строка "max" или "min" (тип оптимизации)
- `intvars` - вектор флагов целочисленности (1 = целое, 0 = любое)

**Возвращает:**
Пошаговое решение с выводом:
1. Канонической формы
2. Симплекс-итераций с таблицами
3. Отсечений Гомори с формулами
4. Двойственного симплекса
...
Вызов:
```
gomory\gomory([[2,3,3,8][1,4,2,7][1,1,5,0]], "max", [1,1,1])
```
...
2. **Ограничения:**
   - Поддерживаются только неравенства `≤`
...
gomory\gomory([[-1,-1,-3][-2,-1,-5][1,2,0]], "min", [1,1])
```
...
gomory\gomory([[1,1,4][2,1,6][3,2,0]], "max", [1,0])
```
